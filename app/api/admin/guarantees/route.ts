...
function sanitizeConfig(input: Partial<GuaranteeConfig> | undefined): GuaranteeConfig {
  ...
  return {
    ...
    inflowCreditFull: Math.max(1, numOr(c.inflowCreditFull, DEFAULT_GUARANTEE_CONFIG.inflowCreditFull)),
    inflowCreditMax: Math.max(0, numOr(c.inflowCreditMax, DEFAULT_GUARANTEE_CONFIG.inflowCreditMax)),
    inflowWindowDays: Math.max(1, numOr(c.inflowWindowDays, DEFAULT_GUARANTEE_CONFIG.inflowWindowDays)),
