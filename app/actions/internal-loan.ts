...
import {
  ...
  LOAN_MESSAGE_MAX_CHARS,
  LOAN_MAX_ATTACHMENTS_PER_MESSAGE,
  LOAN_THREAD_MAX_MESSAGES,
...
// ---------------------------------------------------------------------------
// Loan negotiation thread — a two-way, in-app discussion (messages + document
// uploads) between the borrower and the administrator, attached to the loan and
// visible to both sides before the final approve/decline decision.
// ---------------------------------------------------------------------------

/** A thread as either surface needs it, plus who the viewer is. */
export interface LoanThread {
  approvalId: string
  status: string
  messages: LoanNegotiationMessage[]
}

/** Sanitise a caller-supplied attachment list to trusted, bounded shape. */
function sanitizeAttachments(input: unknown): LoanAttachment[] {
  if (!Array.isArray(input)) return []
  const out: LoanAttachment[] = []
  for (const a of input.slice(0, LOAN_MAX_ATTACHMENTS_PER_MESSAGE)) {
    const at = a as Record<string, unknown>
    const url = typeof at?.url === "string" ? at.url.trim() : ""
    // Only accept uploads that landed on our Blob store (the upload route
    // gates who may write there); reject anything else outright.
    ...
    out.push({
      ...
      url,
      pathname: typeof at.pathname === "string" ? at.pathname.slice(0, 300) : undefined,
...
/** Append a message to a loan's negotiation thread (shared by both surfaces). */
async function appendNegotiationMessage(
  req: Awaited<ReturnType<typeof getApprovalById>>,
  msg: LoanNegotiationMessage,
): Promise<LoanNegotiationMessage[]> {
  if (!req) return []
  const existing = readNegotiation(req)
  const next = [...existing, msg].slice(-LOAN_THREAD_MAX_MESSAGES)
  await updateApprovalPayload(req.id, { ...(req.payload ?? {}), negotiation: next })
  return next
}
...
// ---- Client side (session-gated) -----------------------------------------

/** Read the negotiation thread for a loan the caller owns. */
export async function getInternalLoanThread(approvalId: string): Promise<LoanThread | null> {
  const session = await resolveCurrentSession()
  if (!session?.id) return null
  try {
    const req = await getApprovalById(approvalId)
    if (!req || req.kind !== "internal_loan") return null
    const memberIds = await resolveEnvironmentMemberIds(session.id)
    if (!memberIds.includes(req.userId)) return null
    return { approvalId: req.id, status: req.status, messages: readNegotiation(req) }
  } catch (err) {
    console.log("[v0] getInternalLoanThread failed:", (err as Error).message)
    return null
  }
}

export type PostMessageResult =
  | { ok: true; messages: LoanNegotiationMessage[] }
  | { ok: false; error: string }

/** Post a message (optionally with document attachments) as the borrower. */
export async function postInternalLoanMessage(input: {
  approvalId: string
  body?: string
  ...
  const session = await resolveCurrentSession()
  if (!session?.id) return { ok: false, error: "Your session has expired. Please sign in again." }
  const body = (input.body || "").trim().slice(0, LOAN_MESSAGE_MAX_CHARS)
  const attachments = sanitizeAttachments(input.attachments)
  if (!body && attachments.length === 0) {
    return { ok: false, error: "Write a message or attach a document." }
  }
  try {
    const req = await getApprovalById(input.approvalId)
    if (!req || req.kind !== "internal_loan") return { ok: false, error: "Loan not found." }
    const memberIds = await resolveEnvironmentMemberIds(session.id)
    if (!memberIds.includes(req.userId)) return { ok: false, error: "You cannot post to this loan." }

    const msg: LoanNegotiationMessage = {
      id: newMessageId(),
      from: "client",
      author: session.profile?.fullName ?? "Client",
      body,
      attachments,
      at: new Date().toISOString(),
    }
    const messages = await appendNegotiationMessage(req, msg)

    // Notify the administrator inbox (owner of the platform admin identity is
    // not a normal user, so we surface it as an activity entry) and log it.
    await logActivity({
      action: `New loan discussion message on ${req.id}`,
      category: "Treasury",
      details: {
        summary: `${msg.author} sent a message on internal loan ${req.id}${
          attachments.length ? ` with ${attachments.length} document(s)` : ""
        }.`,
        referenceId: req.id,
      },
    }).catch(() => {})
    ...
    console.log("[v0] postInternalLoanMessage failed:", (err as Error).message)
    return { ok: false, error: "Your message could not be sent. Please try again." }
  }
}

// ---- Admin side (passcode-gated) -----------------------------------------

/** Read the negotiation thread for any loan (administrator). */
export async function getInternalLoanThreadAdmin(
  passcode: string,
  approvalId: string,
): Promise<LoanThread | null> {
  if (!(await adminOk(passcode))) return null
  try {
    const req = await getApprovalById(approvalId)
    if (!req || req.kind !== "internal_loan") return null
    return { approvalId: req.id, status: req.status, messages: readNegotiation(req) }
  } catch (err) {
    console.log("[v0] getInternalLoanThreadAdmin failed:", (err as Error).message)
    return null
  }
}

/** Post a message (optionally with attachments) as the administrator. */
export async function postInternalLoanMessageAdmin(input: {
  passcode: string
  approvalId: string
  body?: string
  attachments?: LoanAttachment[]
  ...
  try {
    ...
    // Let the borrower know there is a new message on their request.
    const ownerId = await resolveDataOwnerIdFor(req.userId).catch(() => req.userId)
    await insertNotification({
      userId: ownerId,
      tone: "info",
      title: "New message on your loan request",
      body: `The administrator sent you a message${
        attachments.length ? ` with ${attachments.length} document(s)` : ""
      } about your internal loan request. Open Treasury to reply.`,
      href: "/dashboard/treasury",
    }).catch(() => {})

    await logActivity({
      action: `Administrator replied on loan discussion ${req.id}`,
      category: "Treasury",
      details: {
        summary: `Administrator sent a message on internal loan ${req.id}${
          attachments.length ? ` with ${attachments.length} document(s)` : ""
        }.`,
        referenceId: req.id,
      },
    ...
    console.log("[v0] postInternalLoanMessageAdmin failed:", (err as Error).message)
    return { ok: false, error: "Your message could not be sent. Please try again." }
  }
}
